...
## Run it
Use Atomic Red Team's safe, reversible test — it's built for exactly this:
```powershell
...
- **Sysmon Event 10 (ProcessAccess)**: `TargetImage: lsass.exe` with `GrantedAccess` like `0x1010`/`0x1410`/`0x143a` — the high-access-rights handle a dumper needs. `SourceImage` names the tool (rundll32/procdump/etc.).
...
Which process accessed LSASS, with what `GrantedAccess` mask, at what time? Was a dump written to disk (Event 11)? How do you distinguish this from legitimate LSASS access (AV, MsMpEng) — what's the discriminator? Given no creds left the lab, what's the *earliest* point you could have alerted?

## Detection engineering
An Event-10 rule where `TargetImage` ends in `lsass.exe` and `GrantedAccess` is in the dumper set, excluding known-good `SourceImage`s. This is one of the highest-value endpoint detections there is — ship it and document the allowlist you had to build.
