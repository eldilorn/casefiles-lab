...
## Story
No PowerShell this time. A trusted, signed Windows binary was used to pull a file from the internet — the technique that slips past "block powershell" rules. Which binary, what did it fetch, and did it execute?
...
- **Sysmon 11** (file create): the dropped `p.exe`, then **Sysmon 1** for its execution — parent is the LOLBin.
...
Command-line rules for `certutil`/`bitsadmin`/`mshta`/`regsvr32` with `http`/`urlcache`/`/transfer` tokens, or Sysmon-3 alerts where the process image is a known LOLBin. Ship one; note it's behavior-based, not signature-based — that's the point.
